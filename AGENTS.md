...
- Do not report a live integration as verified from a simulated provider or local VM. Hetzner startup supports only the explicitly configured image factory; customer endpoints and jobs remain disabled. Image start can incur charges. Run it only after reachable enrollment, current inventory, explicit cheap caps and full cleanup are ready. See `docs/architecture/operator-runtime.md`.
