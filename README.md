...
**Current status: implemented deployment path, unfinished hosted preview.** The production CLI/API/worker already connect customer login, Hetzner provisioning, SSH, Compose and HTTPS routing. A ready public customer instance has not been deployed and verified. An internal reference application passed a real Hetzner deployment with frontend/backend/PostgreSQL, HTTPS, disconnection, persistent update and cleanup; that VM was deleted. Customer Compose and recovery have separate local/native evidence. The [completion plan](docs/COMPLETION_PLAN.md) traces the existing code and prioritizes configuring the hosted service, fixing bounded packaging gaps and simplifying first use. See [the recorded live result](docs/reference-deployment-verification.json) and [the current handoff](docs/CONTEXT.md) for evidence and limitations.
