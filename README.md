...
**Current status: unfinished developer preview.** There is no ready hosted customer endpoint recorded for deploying your own project. An internal reference application passed a real Hetzner deployment with frontend/backend/PostgreSQL, public HTTPS, disconnection, an update preserving data and verified cleanup; that VM was deleted. Customer access, Compose and recovery have separate local/native evidence. The [completion plan](docs/COMPLETION_PLAN.md) puts a usable real customer deployment first, followed by protected provider recovery and the remaining non-Stripe scope. See [the recorded live result](docs/reference-deployment-verification.json) and [the current handoff](docs/CONTEXT.md) for evidence and limitations.
