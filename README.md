...
Read the [architecture and examples](docs/architecture/overview.md) for the current runtime topology, internal request paths, permissions, state, deployment commands and recovery boundaries.

**Current status: real customer deployment verified; no persistent hosted endpoint.** On 2026-09-08, the published CLI authenticated through GitHub against a Hetzner-hosted service, created a customer VM and deployed an ordinary frontend/backend/PostgreSQL project over trusted HTTPS. Browser writes survived CLI disconnection, an update, failed-build recovery and customer/platform reboots. Project-restricted access and revocation also passed. The disposable infrastructure was removed to bound costs. Start with the [customer walkthrough](docs/customer-quickstart.md); the [live verification record](docs/customer-deployment-verification.json), [remaining plan](docs/COMPLETION_PLAN.md) and [current handoff](docs/CONTEXT.md) distinguish this proof from a continuously available service and the remaining recovery work.
...
Managed routing uses `acld route publish/wait/inspect/remove` and `acld domain add/verify`. The public gateway forwards with mTLS to the guest proxy; applications bind to loopback ports. See [HTTPS routing](docs/architecture/https-routing.md) for ownership, update and outage semantics. Generated application hostnames passed real Hetzner/public ACME verification, including recovery after a platform reboot. Customer-supplied domain verification remains a separate provider acceptance step.
...
The [self-host quickstart](docs/self-host.md) packages the API, worker, CLI and PostgreSQL in a pinned runtime image. Its local simulated scenario verifies persisted identity and machines across a restart. Isolated dump and encrypted WAL recovery preserve the selected CLI identity, machine and reservation history while leaving the original installation unchanged. The [customer Compose configuration](docs/self-host-customer.md) packages operator admission, a private CA and shared control HTTPS gateway. Its real hosted run verified GitHub login, revocation and service restart. Restored real control state is [externally fenced](docs/control-recovery.md) before mutators can resume. Off-host storage and final provider recovery remain separate verification steps.
...
Download the [CLI developer preview](https://github.com/yahorbarkouski/agent-cloud/releases/tag/cli-v0.1.0), which includes dependencies, recipes, agent instructions and original license notices. Follow [CLI installation](docs/cli-install.md) to verify its GitHub build provenance and checksums, then unpack it without cloning the repository. The published archive was independently downloaded and verified, including refusal of tampered bytes and a mismatched release reference. `pnpm smoke:cli-release` verifies extraction outside the checkout, offline assets, private login, authenticated API commands and revocation. Development uses `pnpm acld`.
...
Hetzner catalog reads have been verified against a real account. The API refreshes catalog snapshots outside request transactions. Operator image jobs require explicit configuration and start; live customer admission requires explicit operator runtime configuration and spending limits. The first bounded CPX12 image drill reached a snapshot and verifier, exposed a disk-durability failure, and was fully cleaned up. The correction passed local tests, native clone checks and a fresh Hetzner lifecycle through signed publication and selection. Both attempts were fully cleaned up; independent inventory confirmed zero resources. See the [failed drill](docs/research/m1-hetzner-image-drill.json) and [successful drill](docs/research/m1-hetzner-durability-drill.json). The internal reference deployment subsequently passed its full live scenario; customer SSH, commands and general Compose have separate local/native verification. See the current handoff for remaining product scope.
