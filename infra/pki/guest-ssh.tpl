...
{{- else -}}
{
  ...
  "extensions": {},
  "criticalOptions": {{ if eq .Type "user" }}{"force-command": {{ if hasPrefix "hosting-alloc_" .KeyID }}"/usr/bin/sudo -n -- /usr/local/bin/guestctl hosting --json"{{ else if hasPrefix "backup-alloc_" .KeyID }}"/usr/bin/sudo -n -- /usr/local/bin/guestctl backup-dispatch"{{ else if hasPrefix "deployment-alloc_" .KeyID }}"/usr/bin/sudo -n -- /usr/local/bin/guestctl reference --json"{{ else if or (hasPrefix "runtime-alloc_" .KeyID) (hasPrefix "runtime-verify_" .KeyID) }}"/usr/bin/sudo -n -- /usr/local/bin/guestctl inspect --json"{{ else }}"/usr/local/bin/guestctl identity --json"{{ end }}}{{ else }}{}{{ end }}
}
{{- end -}}
