...
import { z } from 'zod';
import { generateGatewayTlsKey, issueGatewayTls, validateGatewayTls } from '@agent-cloud/pki';
import { publicGatewayControllerSchema } from './controller.js';
import {
...
export async function runGatewaySetup(args: string[]) {
  try {
    process.stdout.write(JSON.stringify(await setupHostingGatewayCommand(args)) + '\n');
  } catch {
