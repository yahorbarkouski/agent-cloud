...
type EnrollmentPorts = {
  ...
  seal: BootstrapSeal;
  signer: Signer;
  probe: ReturnType<typeof createGuestProbe>;
...
async function ownedAddress(
  db: Database,
  provider: MachineProvider,
  context: Awaited<ReturnType<typeof loadBootstrap>>,
) {
  ...
  async function enroll(value: GuestEnrollmentInput): Promise<IssuedGuestIdentity> {
    ...
    const result = await withMachineLock({
      ...
      work: async (db) => {
        ...
        const address = await ownedAddress(db, ports.provider, context);
