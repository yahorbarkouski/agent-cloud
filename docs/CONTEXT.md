...
Root branch `yahor/agent-cloud`, private origin https://github.com/yahorbarkouski/agent-cloud.git. Last pushed checkpoint521162403960e40ceffbacc552e7c1b7a2041131 integrates customer access storage. Exact Linux CI34141499939 passed. The sole canonical append-only decision trail is `docs/DECISIONS.tsv` here.

Isolated worktree `.local/customer-ssh`, branch `yahor/customer-ssh`, was fast-forwarded to5211624. Its uncommitted customer signer draft changes `packages/pki`, the guest SSH template and new native fixture scripts. It has no provider credentials. Keep root's newer progress/trail when integrating. Do not edit applied migration0020.
...
The reproducible fresh-CA smoke passed in `.local/m2-customer-signer-native-reproducible-second.log`: exact positive certificate, substituted key rejection, malformed/unsigned claims rejection, no expansion from request options, near-expiry authority, existing inspectors, token-free CA output and full local cleanup. The first run failed because the test expected403 but the native template returns400; it also cleaned up. An earlier script typecheck caught an unused import before native execution. No native customer SSH login proof yet. Next add signer lost-response/cancellation and workspace cleanup checks, focused policy tests, review and full verification before exporting or integrating this draft.
...
API session78078 and worker72282 run `node --env-file=.env apps/control/dist/{api,worker}.js`, logs `.local/simulated-{api,worker}-safe-pki.log`. API4319. Original CA https://localhost:9449,256MiB/.5CPU, Step0.30.6/step-ca0.30.2. Its customer template is still the committed restricted template, not the isolated draft. No OrbStack guest, temporary tunnel, paid server or snapshot exists.
