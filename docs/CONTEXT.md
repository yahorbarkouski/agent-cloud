...
Sanitation implementation4653f79 is locally verified, reviewed, committed and pushed. Linux CI34075101498 passed in2m12s, including clean setup and native PKI/SSH/enrollment checks. Fresh full smoke73319 PASSED all installer/data/refusal checks, real interrupted preparing retry, protection of new Docker volume data after unexpected restart, log cleanup, rejected preparing clone and two positive clone enrollment/runtime/reboot checks. Both125-file token scans had zero matches; machine IDs, SSH/TLS keys and allocations differ. Exact public results in docs/research/m1-image-sanitation-verification.json. Image dev-73aed28a0aa7/digestd0bf1a02b1751a9ed8b49be47f2c9cbd0683248141d137b8b55bca0055e76f33. Final orb inventory[] and all3ownership records absent. No local VMs remain; no cloud resources were created.

Full check+format42939 passed121tests/18files, typecheck/lint. Native enrollment9311 passed actual guest library lost-response/replay, Smallstep and OpenSSH/TLS with fixture activation/simulated provider. Final shell syntax, UUID boundary and customer-skill validation passed. Installer embedded JS has only formatting changes after final VM build. Final formatting and diff checks passed. Do not repeat VM or control tests without new implementation changes.

Earlier16252 proved the preceding revision. Native10176 reached real preparing interruption but test Docker restart hit start-limit-hit; fixture now resets counters. Diagnostic43606 proved recovery. Native52823 passed recovery/log/refusal checks but my diagnostic orb run started its deliberately stopped source; child refused before creating a positive clone. That builder and unused child record were deleted. AGENTS now forbids VM commands while a smoke is active. All earlier failure state has been removed. Eight sanitation decision rows preserve this trail; reviewer guest_design_judge on gpt-5.6-sol completed the final artifact/trail audit in docs/research/m1-image-sanitation-review.md. No remaining code blocker was found within the documented exclusive fresh-builder scope. No transcript directory supplied.

Next implementation boundary: full public input provenance, recorded provider snapshot submission/result/cleanup, certificate renewal and operator recovery, then production worker activation and a bounded cheap Hetzner drill. Image manifest currently pins bundle/components/trust but not installer/service inputs; scripts/build-guest.ts has separate SHA256SUMS. See image-sanitation.md next-publication section. Hetzner label values max63characters, so use build ID labels and retain64-hex digest in release metadata. No base DB/control/API/worker changes in sanitation.
...
2. Sanitation/cloned identity proof is complete. Continue public input provenance, snapshot ownership/cleanup, certificate renewal and operator recovery. Production createTasks/API/worker configuration still cannot enable GuestProvisioning; wire it when prerequisites exist. Then bounded cheap Hetzner build/boot/cleanup.
...
Sanitation architecture is in docs/architecture/image-sanitation.md; guest-image.md retains official cloud-init/Hetzner references. Never use orb stop without an explicit owned machine name because omission stops all OrbStack services.
