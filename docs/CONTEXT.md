...
The next verified source checkpoint adds operator-only blocked-upload recovery. `pnpm backup:recover inspect <backup-id>` returns an immutable receipt digest; `apply <private-request.json>` verifies only the recorded upload, including after retention expires, then restores captured state/accounting and queues source staging cleanup. It never repeats capture/PUT/restore, survives original-grant revocation and retries idempotently. No reader/keyring/provider/guest runtime is loaded. Adapter recovery still checks exact identity, metadata, bytes and minimum retention; prepare/upload still demand future retention. A bounded recovery review found no blockers.
...
Next acceptance: insufficient control-worker scratch is refused before backup transfer or restore download, retains ownership without exhausting attempts for work that never started, and resumes once capacity returns. Then move to explainable resource usage and self-hosted installation/recovery instead of expanding backup prerequisites. Hetzner Object Storage access remains externally blocked.
