...
M2 authority commit2ece39a4304f8c27ddba92e5bdf5207b32474cd7 is pushed on `yahor/customer-ssh` and integrated into root. One bounded SQL ancestry snapshot, PostgreSQL expiry after traversal and the earliest parent horizon replace separate host-clock reads. Eleven targeted tests and429full tests pass. Review resolved its snapshot finding. Exact Linux CI34136058956 passed checks, formatting and fresh native smokes on retry after registryHTTP500 prevented the first PostgreSQL setup. Root integration typecheck/build and the actual CLI smoke passed after restarting API/worker, including full simulated cleanup. M1 evidence commit42a5c61 also passed exact Linux CI34138682426. Session storage checkpoint49f949f is integrated at5211624 and exact Linux CI34141499939 passed. It passes445tests/38files, typecheck/lint in103.50s and formatting. Independent review resolved all three original blockers and withdrew a stale URL-normalization concern after checking current source and PostgreSQL. Strict SQL admission validation, immutable signing receipts, actual signing races, rollback, closed gateway tuple uniqueness and contract/SQL parity are covered. Migration0020 is applied to the main local DB; all21 hashes match. Rebuilt/restarted API and worker passed actual CLI lifecycle and cleanup forvm_cae6e7df-7048-4998-90a5-633cb7688501. It is not yet a usable access service.
...
## Customer certificate signer verified locally

The isolated customer signer passed448tests/39files, typecheck/lint in108.60s, formatting and a fresh native CA/OpenSSH smoke. It binds each certificate to the signed key, allocation, gateway sources and exact database-derived expiry, with PTY only. One POST under lost/invalid responses and cancellation, pre-submit failure classification, workspace cleanup, corrupt-signature rejection and probe compatibility are covered. Independent review resolved the submission-state finding. The native fixture uses private NAT ranges and disables temporary source bans only to exercise repeated negative logins; no provider or production-image claim follows. See [verification](research/m2-customer-signer-verification.json) and [review](research/m2-customer-signer-implementation-review.md). Signer checkpointb441661 is pushed and fast-forward integrated into root. Exact Linux CI34145188845 passed. Root integration typecheck/build, native PKI and restarted actual CLI lifecycle/cleanup passed. The access-capable image, API, durable issuer, gateway and CLI remain open.
...
- [ ] Implement: shared grant authority2ece39a passed429tests and exact Linux CI34136058956 on retry. Session contracts and0020 passed445tests and independent storage review. The migration is applied locally and immutable. Admission/current authority, issuance, gateway, guest and CLI remain.
...
## Machine cleanup verified locally and in Linux CI
...
- [x] Verify exact-commit Linux CI34107140639 on9913342643ae10f8dadbd400cd0a1270cf902ba0, including fresh PKI, native smokes and cleanup. Actual local CLI smoke and all17 migration hashes also passed.
- [x] Run the first bounded CPX12 builder/snapshot/verifier attempt and verify complete cleanup. Build `bbb28cd4-aca1-4a94-87cf-364728bf6771` reached a verifier, which could not enroll because installed image files were empty. Rescue inspection followed cancellation; all seven owned resources and local access were removed. Actual provider inventory was zero at10:00:18Z. [Live drill evidence](research/m1-hetzner-image-drill.json) preserves the failed attempt separately from later verification.
...
- Migration 0016 is applied and immutable; `pnpm db:check` verified all 17 hashes. The base database has zero active allocations, simulated VMs/IPs and open image builds. No paid resource was created. Evidence and limits are in `research/m1-image-use-verification.json`. Independent gpt-5.6-sol artifact/trail review found no blocker; report `research/m1-image-use-review.md`. Implementation `ba42de027bab4928127a0d98979bb7c1989b9377` is committed and pushed. [Linux CI 34101902876](https://github.com/yahorbarkouski/agent-cloud/actions/runs/34101902876) passed in 3m41s, including frozen installation, full checks, formatting, fresh PKI, native PKI/SSH/enrollment and cleanup.
...
Customer access storage is integrated at5211624 with exact Linux CI34141499939 passing. The main development database has immutable migration0020, all21 hashes match, and actual CLI cleanup passed. Customer signer work is isolated and uncommitted. Its first native positive exposed default Smallstep token logging; setup now disables that access logger for existing and new CAs. The main CA was recreated without changing keys, and native SSH/TLS signing verified no token output. Access admission, gateway and customer CLI remain open.
