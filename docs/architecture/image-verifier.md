...
Customer SSH is part of the admitted image identity. When a manifest declares numeric `customerSsh: 1`, both verifier and customer-allocation bootstrap SQL guards preserve that exact field, and the verifier result requires exactly `customerSsh: {kind: "ok"}` alongside the established checks. Keep fixtures aligned with current guest manifests: legacy-only fixtures previously hid a real publication failure. Migration 0028 updates the existing guards without rewriting applied migrations or relaxing legacy identity/proof checks.
