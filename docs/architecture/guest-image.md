...
A local VM pass does not complete M1. Runtime readiness is implemented and described in [guest-runtime.md](guest-runtime.md). Sanitation and local clone verification are described in [image-sanitation.md](image-sanitation.md). Provider snapshot ownership/cleanup, production worker activation, ongoing renewal, explicit operator recovery and the bounded live drill remain. Application deployment, customer access, routes, backups and restore belong to later milestones.
