...
Before downloading an object or consuming a preparation attempt, the worker authenticates its wrapped data key against the recorded account, backup UUID and manifest. A missing, unreadable or wrong wrapping key leaves the same restore pending, with `state.waitingFor: "backup_key"` visible through `acld backup restore-inspect`. The target remains isolated and billable. Recover the matching private keyring from its independent copy; the worker reloads it on the next reconciliation without a restart or a new restore UUID. Revoking the admitting credential still blocks the operation. A damaged key envelope also fails this authentication, so verify the protected database receipt if recovering the trusted key does not resolve it. No archive bytes reach the target until full decryption succeeds.

Keep a private copy of every required key version outside the control host's failure domain. The control database stores encrypted data-key envelopes, not these wrapping keys. After rotation, preserve older versions until no retained backup needs them. Recover the operator-owned keyring to its configured absolute path with owner-only permissions, using a temporary private file and an atomic rename. Do not paste it into a CLI argument, issue, log or agent conversation. If recovery will take time, the customer can inspect or explicitly destroy the isolated target through the ordinary machine commands; destroying it does not delete the protected source backup.
...
The independent wrapping-key recovery scenario passed in `.local/backup-key-native-1.log`. The customer admitted one restore, observed `waitingFor: backup_key` through the CLI during six missing-key and six wrong-key worker refusals, recovered the matching private copy, then verified PostgreSQL count1, identical declared files and count2 through the moved HTTPS route. No guest backup credentials were issued while keys were unavailable. Integration tests separately verified zero object downloads, unchanged attempt counters, revocation and inspection of submitted SQL without keys. Both VMs, MinIO and the temporary private copy were removed; `.local/backup-key-cleanup-1.json` records checked absence. The private copy lived outside active configuration on the same test host, so this does not establish off-host durability or Hetzner S3 recovery.
