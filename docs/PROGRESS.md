...
The isolated customer signer passed448tests/39files, typecheck/lint in108.60s, formatting and a fresh native CA/OpenSSH smoke. It binds each certificate to the signed key, allocation, gateway sources and exact database-derived expiry, with PTY only. One POST under lost/invalid responses and cancellation, pre-submit failure classification, workspace cleanup, corrupt-signature rejection and probe compatibility are covered. Independent review resolved the submission-state finding. The native fixture uses private NAT ranges and disables temporary source bans only to exercise repeated negative logins; no provider or production-image claim follows. See [verification](research/m2-customer-signer-verification.json) and [review](research/m2-customer-signer-implementation-review.md). Code integration is next; the access API, durable issuer, gateway and CLI remain open.
