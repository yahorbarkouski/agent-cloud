...
`infra/compose/customer.yaml` runs the existing customer API, worker, PostgreSQL, private certificate issuer and both gateways on one Linux host. Caddy serves the control API, SSH WebSocket upgrades and application HTTPS on ports80/443. The API and access gateway listen on loopback inside a shared network namespace. PostgreSQL and the certificate issuer have no host ports. API and worker have outbound access to Hetzner and guests; the database remains on an internal network. Persistent services restart after a host reboot. Operator and retention commands run once.
...
| `control`    | `control.env`, provider token, customer runtime JSON, access/hosting/backup configuration and required signer/trust files. Read-only in API and worker.                                                 |
| `auth`       | `github.json` with the OAuth application credentials and device-login configuration. Read-only in the API only.                                                                                         |
| `ca`         | Smallstep issuer configuration, intermediate and SSH signing keys, issuer password and CA database. Mounted only in the CA service. The offline root private key stays outside this directory and host. |
...
Initialize a new installation's PKI using the existing `pnpm setup:pki` procedure in an isolated preparation directory with the pinned Step binary and `infra/pki` templates. Preserve its identity on retries. Copy only the generated `issuer/` tree into `ca/`, add `ca` to `config/ca.json`'s `dnsNames`, and set the mounted tree's ownership to UID/GID1000. Keep the offline root and provisioner passwords in separate protected operator storage. Configure signers to use `https://ca:9000` with the generated public root. Do not expose port9000 or mount the CA directory into the API or worker. Copy only each signer's required provisioner password and public trust into its configuration directory.

The runtime image includes `runtime-identity`, `image-build` and `setup-gateway` commands with the same arguments as the repository scripts. Run identity initialization once with an explicit private writable mount. For a fresh guest image, set `ACLD_IMAGE_TRUST_DIRECTORY` to this installation's generated `public/` directory when running `pnpm build:guest`, then retain its immutable manifest directory. Use `setup-gateway` as a one-off command on the CA network with temporary access to the gateway provisioner password; runtime gateways receive only their issued client identity and public trust. Never replace an existing installation's trust or runtime identity during an upgrade.
...
Use `operator customer disable <github-user-id>` to revoke the admission anchor and descendants. Renewal takes a private request with the current `expectedAnchorGrantId`; inspect first and retain the returned receipt. Existing revoked credentials stay revoked after renewal. Run optional retention with its separately scoped credentials: `customer_compose run --rm --no-deps retention`. Schedule that bounded command explicitly if recurring retention is needed.
